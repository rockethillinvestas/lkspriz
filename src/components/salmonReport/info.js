...
class SalmonInfo extends Component {
  calculateChange(){
    ...
    return (<h2 style={rounded > 0 ? {color: 'green'} : {color: 'red'}} >{rounded > 0 ? '🞁' : '🞃'}{rounded}%</h2>);
  }
  render () {
    ...
    return (<div className="salmon-info"> 
    <div className="last-price">
    <h3>
      Siste pris:  
    </h3>
    <h2>
      {this.props.data.value[(this.props.data.value.length/2) - 1]} kr per kg
    </h2>
    </div>
